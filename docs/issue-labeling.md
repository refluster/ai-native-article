...
| `wf:lane:implement` \| `wf:lane:design` \| `wf:lane:operator` | the `issue-triage` Cadence (`workforce/skills/issue-triage/issue-triage-post.mjs`) | The worker lane this issue was dispatched to, per [workforce ADR-0022](../workforce/docs/adr/adr-0022-issue-to-merge-flow.md) — `implement` → `issue-implement` (ren), `design` → `issue-design` (dario), `operator` → a human, named explicitly. **One issue, one lane**: the Cadence removes any other lane label as it stamps. Deliberately absent from `.github/labels.json` and created on demand by the Cadence (same colour every time, §4.2), like `owner:` below. Never hand-apply — re-lane by re-running triage, or say why in a comment and let the next fire re-examine it. Note this is a different axis from `wf:ready` / `wf:in-progress` / `wf:blocked` in §1.2, which are workflow *status*, not *routing*. |
| `wf:owner:<slug>` | the `issue-triage` Cadence | The workforce persona accountable for the lane assignment. A persona slug, **not** a GitHub account (ML-012) — so it never becomes an `@`-mention. Created on demand for the same reason `owner:` is: the slug set is open-ended. |
