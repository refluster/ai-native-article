...
| R-16 | Base-path consistency gate — every hand-written base-path literal (`index.html`, `404.html`, `manifest.webmanifest`, `sw.js`, `robots.txt`) must agree with `SITE_BASE_PATH`, the one constant vite and the router read. The comparison is **directional**: a literal must sit under the base *and* not repeat a base segment after it, because a plain prefix test is vacuous at base `/` — where every root-absolute URL trivially matches | `node scripts/check-base-path.mjs` (comparisons unit-tested in `scripts/lib/base-path-literals.test.mjs`) | `ci.yml` | ✅ added 2026-08-24 (ML-031) |
| R-17 | Live base-path smoke — the deployed site's own URL must serve the app shell; every same-origin asset it references, the service worker's precache `SHELL` and the manifest's icons must return 2xx; and a deep link must land in *our* SPA fallback with *our* base. So "green deploy, blank page" is observed rather than reported by a reader. Exits **2** (check could not run) rather than 1 (site broken) when a request never completes, so a proxy blip is not filed as an outage. Post-deploy it is given the `dist/` it just published (`R17_EXPECT_BUILD_DIR`) and **settles on build identity**: until the served shell references that build's hashed assets it retries (`R17_SETTLE_ATTEMPTS`, ~5 min) and never passes, because Pages serves the *previous* build for minutes after a green deploy — and that stale build is normally healthy, so waiting on failure alone would sign off a deploy it never looked at. Exhausting the window exits **2**, never 0 (ML-032) | `node scripts/check-live-base-path.mjs` | `deploy-article-site.yml` (final step, after the gh-pages publish) **and** `corpus-freshness.yml` (daily 21:00 UTC) — **not** a PR gate: it asserts deployed state, which no diff controls | ✅ added 2026-08-24 (ML-031) |
| R-18 | Cadence write-script read-back gate — every bundled write-script that POSTs a caller-supplied `--body-file` to the shared feed endpoint must verify by read-back (`verifyReadBack()`) before exiting 0, so a batched fire's shared-scratchpad race publishes loud (exit 2), never a silent misattribution | `node scripts/check-cadence-readback-guard.mjs` | `ci.yml` | ✅ added 2026-08-17 (ML-028); population discovery found 14 pre-existing gaps, tracked in the script's own `KNOWN_GAPS` (3 pending #546, 11 pending follow-up) |
| R-19 | W-3 budget runway audit — every non-archived agent's effective monthly cap must cover the modelled monthly burn of its orchestrator-owned bindings (fires in a fixed 30-day window × the skill's declared `cost_class`), because since #661 the orchestrator charges that model per fire and refuses to dispatch past the cap — so a cap the cadence outruns is the day the agent stops, not a budget. Caps and bindings are DDB config, not git, so this reads the live roster and is a scheduled audit like R-13/R-15, **not** a PR gate; the write-time half (`W3-runway` in `workforce/lambdas/shared/agent-config.ts`) refuses the state at the source | `node workforce/scripts/check-budget-runway.mjs` (`npm run workforce:budget-runway`) | `workforce-budget-runway.yml` (daily) | ✅ added 2026-09-14 (ML-038) |
...
**Policy.** R-1, R-3, R-4, R-5 and R-6 were guards on the GAS L1→L4 engine; they were **retired 2026-06-28** when that engine was removed and generation moved to the workforce `article-level{2,3}` cadences. Their editorial-integrity job did not disappear — it moved to **W-1** (the generation-time guard in `article-level{2,3}/publish-notion.mjs`, owned by the workforce governance) plus **R-10** (the deploy-time gate). R-2, R-8, R-9, **R-10** must stay green for `deploy-article-site.yml` to ship. R-10 is the *deploy-time* twin of the *generation-time* W-1 guard: W-1 stops bad content from being written to Notion; R-10 stops it from being published to gh-pages. The `article-health` skill (R-7) is advisory but should be run after any generation change and after every user-reported content issue. R-11 requires an L1-document edit (a framework law in §3.1, this doc / design-policy, the workforce statute docs, or any ADR under `docs/adr/` / `workforce/docs/adr/`) to cite the law it touches — so following the relevant ADR when implementing is mechanical, not optional. R-12 keeps the two governance registries ([memory-lint-backlog.md](memory-lint-backlog.md), [risk-acceptance-ledger.md](risk-acceptance-ledger.md)) machine-parseable. **R-16 and R-17 are a pair, and neither is sufficient alone.** R-16 proves the base-path literals agree with *each other*; R-17 proves they agree with *reality* — which repo currently holds the custom domain is a GitHub Settings fact no diff can see. PR #606 moved every literal to `/` consistently (R-16 would have stayed green) and the live site 404'd its whole shell for a week; conversely R-17 alone cannot run on a pull request, because it inspects deployed state. **R-17's detection budget, stated so it is a check with an objective rather than a check with a cadence** (the gap the #619 panel named): a base-path regression introduced *by a deploy* is caught by that deploy's own final step — **≤ ~6 min** (propagation, plus a settle window of up to ~5 min, plus the check itself); one introduced *outside* a deploy — a GitHub Pages Settings or DNS change, which is how the apex was lost in the first place — is caught by the daily run, so **≤26h** worst case (21:00 UTC, plus the 1–2h GitHub routinely adds to a schedule). The daily run is also the fallback whenever a post-deploy run is *inconclusive* (exit 2: propagation overran the window), so the ≤26h number is the one to quote when the post-deploy run did not conclude. The residual hazards that make the second number matter are logged as RAL-004 and RAL-005. R-13 is the mechanical half of the §4.4 two-outcome contract: a daily scheduled sweep that escalates any open PR the autopilot left in a non-terminal state (promoted from ML-009 per the §6.1 ratchet). The full operating notes for R-10…R-13 live in [governance-mechanisms.md §2.1](governance-mechanisms.md#21-how-to-operate-each); the decision records are [ADR-0001](adr/adr-0001-self-driving-governance-mechanisms.md) and [ADR-0004](adr/adr-0004-governance-consolidation.md).

**Loosening.** Tightening any active R-rule (R-2, R-7…R-17) is L2 work and an agent may do it freely. **Loosening, disabling, or retiring any of them requires operator approval** — drop the line in chat with the rationale, wait for explicit yes. (The 2026-06-28 retirements above were operator-approved as part of the GAS-cleanup refactor.)
