...
| R-19 | W-3 budget audit — the roster's modelled monthly burn (each non-archived agent's orchestrator-owned bindings: fires in a fixed 30-day window × the skill's declared `cost_class`) must stay under the aggregate W-3 ceiling (USD 600, `workforce/docs/governance.md` §2). Per-agent figures are advisory since workforce ADR-0037 — an agent over its figure is *listed*, never a failure, and never a refused fire. Caps and bindings are DDB config, not git, so this reads the live roster and is a scheduled audit like R-13/R-15, **not** a PR gate | `node workforce/scripts/check-budget-runway.mjs` (`npm run workforce:budget-runway`) | `workforce-budget-runway.yml` (daily) | ✅ added 2026-09-14 (ML-038); advisory per-agent semantics 2026-09-14 (ADR-0037) |
