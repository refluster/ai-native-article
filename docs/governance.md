...
| R-16 | Base-path consistency gate — every hand-written base-path literal (`index.html`, `404.html`, `manifest.webmanifest`, `sw.js`, `robots.txt`) must agree with `SITE_BASE_PATH`, the one constant vite and the router read | `node scripts/check-base-path.mjs` | `ci.yml` | ✅ added 2026-08-24 (ML-031) |
| R-17 | Live base-path smoke — the deployed site's own URL must serve the app shell and every same-origin asset it references must return 2xx, so "green deploy, blank page" is observed rather than reported by a reader | `node scripts/check-live-base-path.mjs` | `corpus-freshness.yml` (daily 21:00 UTC) — **not** a PR gate: it asserts deployed state, which no diff controls | ✅ added 2026-08-24 (ML-031) |
