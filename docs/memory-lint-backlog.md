...
| ML-009 | Every escalation of a PR to a human MUST stamp `autopilot:needs-human`, regardless of *who* escalates. The pr-autopilot skill enforces this mechanically on its write-path (`workforce/skills/pr-autopilot/pr-autopilot-post.mjs` adds `ESCALATION_LABEL` from the `--needs-human` flag OR the hidden `<!-- autopilot:needs-human -->` body marker), but a **session-driven** escalation — a Claude Code / persona-in-session reviewer handing a PR to the operator without going through that script — bypasses the label entirely, so the operator's `is:open label:autopilot:needs-human` queue silently misses it. Candidate guard when it recurs: a scheduled lint that flags any merged/open PR carrying an autopilot-verdict marker (or an L0/L1-touch escalation) without the label; or routing all human hand-offs through the labelled post path. | #358 (project-ind) touched the L0/L1 path `workforce/docs/data-model.md`; pr-autopilot correctly escalated to the operator in-session (R-N10 / adr-0010) but the PR was not labelled `autopilot:needs-human` until the operator flagged the miss (2026-06-22); label applied retroactively before the operator-approved merge. | 1 | watching | — |
