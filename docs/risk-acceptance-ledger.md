...
| RAL-005 | R-17's post-deploy run does not conclude instantly, and can decline to conclude at all. It waits for GitHub Pages to serve the build the run published (identity settle, ~5 min window), so a bad deploy is reader-visible for up to ~6 minutes before the gate reddens; and if propagation overruns the window the run exits **2 — inconclusive**, not 1, leaving detection to the daily run (≤26h). | detection-latency / accepted | The alternative is worse in both directions and both were reproduced: failing immediately on a stale-but-200 shell is a false red on every deploy (the #619 incident, ML-032), and retrying merely until nothing fails is a **false green** — the stale build is normally healthy, so the gate signs off a deploy it never looked at (#620 review, Farah F1 / Owen O1, reproduced). Refusing to conclude when we have not seen the published build is the honest third option (C-4: a broken state must not silently report success). ~6 min is bounded by `R17_SETTLE_ATTEMPTS` x `R17_SETTLE_DELAY_MS` in `deploy-article-site.yml`. | Re-evaluate if a post-deploy run exits 2 more than once — that means Pages propagation routinely overruns 5 min and the window (or the approach) needs revisiting. Raised by the #620 review panel (Farah F2/F4). | agent-proposed 2026-08-24, awaiting operator sign-off |
| RAL-006 | We do not know our own regulatory classification. Whether an editor-less, AI-only publisher is a "provider" / "deployer" / "responsible party" under the EU AI Act's Article 50 transparency duties (and the analogous US state regimes) is unresolved, and — per the operator's 2026-09-07 decision not to retain outside counsel — will stay unresolved. Noor's consolidated question (2026-08-22) is the framing; no answer is being sought. | legal-classification / unresolved | [ADR-0006](adr/adr-0006-publication-disclosure-posture.md) makes the answer non-blocking rather than known: we claim no exemption and no scope threshold anywhere, and build to the strictest reading (D-1 disclosure floor, D-2 no automated decisions about natural persons, D-3 named accountability). Every branch of the classification demands the same conservative action, so the unknown does not change what we build — only whether we were obliged to. Authoring the answer in-house is refused (the org does not write its own legal opinions, and a wrong self-classification's only use would be to justify claiming an exemption). The operator is the legal person bearing this residual; this signed row is what replaces the counsel opinion. | When the org gains users at scale, revenue, or human employees; when a regulator publishes guidance that changes the calculus; or if the operator later retains counsel. Also re-read on 2026-12-02, when the Art.50 grace window for already-marketed systems closes. | agent-proposed 2026-09-07, awaiting operator sign-off |
