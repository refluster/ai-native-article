...
project. `.clasp.json` at the repo root sets `rootDir: "newsletter/gas"`, so
`clasp push` uploads exactly these files:

- `newsletter/gas/appsscript.json` — the manifest (timezone, OAuth scopes, web-app settings)
- `newsletter/gas/src/Code.gs` — all handlers
...
`package.json` and `newsletter/app/src/lib/gas-config.ts`.

## Adding a new API scope

1. Add the GAS call to `Code.gs` (e.g. `ScriptApp.newTrigger(...)`).
2. Add the OAuth scope URL to `oauthScopes` in `newsletter/gas/appsscript.json`.
