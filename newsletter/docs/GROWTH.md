...
L2's job: compress 1–5 L1 articles into a coherent Japanese blog post that stays faithful to sources while producing a readable narrative for software/design engineers at large Japanese manufacturers (per [workforce/skills/article-level2/SKILL.md](../../workforce/skills/article-level2/SKILL.md)).
...
L3's job: find the principle that connects ostensibly unrelated L2s. A summary fails this rubric by definition. Reference: [workforce/skills/article-level3/SKILL.md](../../workforce/skills/article-level3/SKILL.md).
...
Shipped: `scripts/lint-design-tokens.mjs` in CI blocks raw hex and non-zero border-radius in the apps' `src/`. Tokens live only in [tailwind.config.ts](../app/tailwind.config.ts). [DESIGN.md](DESIGN.md) is the spec.
