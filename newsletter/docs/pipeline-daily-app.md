...
Files: `newsletter/gas/src/Code.gs` (`handleL1List` now returns `createdAt` =
...
Handler: `handleL2Batch` in `newsletter/gas/src/Code.gs`.
...
1. Read `newsletter/app/public/posts/manifest.json` from GitHub. Build set of published slugs.
...
Handler: `setupDailyTriggers()` in `newsletter/gas/src/Code.gs`. `appsscript.json`
...
| `newsletter/app/public/posts/manifest.json` | Source of truth for published L3s | `L4_BATCH`/`L4_PUBLISH` |
