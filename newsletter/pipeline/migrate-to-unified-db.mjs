 ...
 *
 * Usage:
 *   node --env-file=.env newsletter/pipeline/migrate-to-unified-db.mjs \
...
// Mirror the legacy DB ids hard-coded in newsletter/gas/src/Code.gs so this script
...
function buildSlugIndex() {
  const dir = join(ROOT, 'newsletter', 'app', 'public', 'posts')
  const map = new Map()
