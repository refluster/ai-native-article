...
// Public (outside AuthBoundary), reached from the public header beside
// Docs.
//
...
import { CARD, CHIP, CHIP_ACTIVE, INLINE_ACTION, KICKER } from '../components/public/styles';
...
const PAGE_BUTTON =
  'font-wfmono text-[11px] uppercase tracking-[0.14em] px-4 py-2 rounded-full border border-wf-outline text-wf-on-surface hover:border-wf-primary hover:text-wf-primary transition-colors disabled:opacity-40 disabled:hover:border-wf-outline disabled:hover:text-wf-on-surface';
  ...
  return (
    <PublicShell width="wide">
      <PageHero
        ...
        aside={<LanguageToggle value={lang} />}
        lede={
          <>
      ...
      <div className="grid gap-10 lg:grid-cols-12 pb-6">
        <div className="lg:col-span-8 xl:col-span-9 min-w-0">
          <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-3">
            <div className="flex items-baseline gap-3">
              ...
              {activeTag && (
                <button type="button" onClick={() => patchParams({ tag: null, page: null })} className={INLINE_ACTION}>
                  Clear
                </button>
              )}
            </div>
            <label className="flex items-center gap-2 border border-wf-outline-variant bg-wf-surface-container-lo rounded-full px-3.5 py-1.5 focus-within:border-wf-primary transition-colors">
          ...
          <div className="mt-6">
            {manifest.error && (
              <div className={`${CARD} p-4 text-sm text-wf-throwing`}>
          ...
          {totalPages > 1 && (
            <nav className="mt-8 flex items-center justify-between font-wfmono text-[11px] uppercase tracking-[0.14em]" aria-label="Pagination">
              <button type="button" onClick={() => goToPage(safePage - 1)} disabled={safePage === 1} className={PAGE_BUTTON}>
                ← Newer
              </button>
              <span className="text-wf-on-surface-variant">
                Page {safePage} / {totalPages}
              </span>
              <button type="button" onClick={() => goToPage(safePage + 1)} disabled={safePage === totalPages} className={PAGE_BUTTON}>
                Older →
              </button>
            </nav>
          )}
        ...
        <aside className="lg:col-span-4 xl:col-span-3 space-y-8 lg:sticky lg:top-6 lg:self-start lg:max-h-[calc(100vh-3rem)] lg:overflow-y-auto">
          <section>
                ...
                {visibleTags.map(t => (
                  <li key={t.name}>
                    <button
                      ...
                      className={`${activeTag === t.name ? CHIP_ACTIVE : CHIP} py-1`}
                    >
            ...
            {tags.length > RESEARCH_TOP_TAGS && (
              <button type="button" onClick={() => setShowAllTags(v => !v)} className={`mt-3 ${INLINE_ACTION}`}>
