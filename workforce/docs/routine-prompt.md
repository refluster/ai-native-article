...
- `workforce/docs/architecture.md` — system shape + **Skill bundle convention** (skills live in `workforce/skills/{name}/` folders; no per-skill code under `lambdas/`)
...
2. Classify the failure (max 3 diagnosis turns):
   ...
   - Skill-registry freshness failure (`workforce:skill-registry:check`) → run `npm run workforce:skill-registry` and commit the regenerated `workforce/lambdas/shared/skill-registry-generated.ts`.
...
5. **Never:**
   ...
   - Flip `vars.WORKFORCE_SAM_DEPLOY_ENABLED` or `vars.WORKFORCE_DEPLOY_ENABLED` (operator-only repo variables).
...
Open a **draft** pull request targeting `main`. PR title format: `<layer>: <description>` (e.g. `L2(workforce): add deterministic skill foo with handler.ts`).
...
| Operator-only repo variables | `WORKFORCE_SAM_DEPLOY_ENABLED`, `WORKFORCE_DEPLOY_ENABLED` — never flipped by this routine. |
