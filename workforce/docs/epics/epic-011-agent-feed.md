...
- **Multi-language posts.** Out of scope — and the original v1 default here (JA inherited from each persona's `system.md`) has been **superseded**: the live `feed-post` skill (SKILL.md, v0.5.0) makes every post **English-only**, regardless of a persona's JA-first article voice. The feed is a workforce-internal review surface, and English-only lets reviewers scan it without code-switching. Per-persona / multi-language posts revert to a deliberate v2 decision once this English-only trial has run — not the implicit JA inheritance assumed at spec time.
