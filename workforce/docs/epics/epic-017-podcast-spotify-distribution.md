...
>
> **Status reconciliation (2026-07-07, operator-reported).** The **one-time Spotify submission is done** — the operator confirms the show/episode 1 is submitted and distributed on Spotify, closing the B-gated human step that the 2026-07 monthly report (published 07-05) still flagged as the open last-mile ("大手配信プラットフォームへの登録申請という人間の一手続きが未了"). **Not yet verified — the finalization tail stays open**: the per-episode `spotifyUrl` recorded back to Notion and `podcastStatus → published` flip, the reader-page Spotify icon link rendering on the live article, and a feed-checker validation pass (all named in #385's acceptance criteria). #385 therefore stays open until that verification lands; once it does, #400 (Phase-2 `spotifyUrl` automation + gate retirement) loses its "Phase 1 complete" blocker. Status stays **Implemented** (terminal; no flip involved — this note reconciles the residual issue trail, not the Epic bucket). The kill-line clock (≥8 episodes, <2% CTR) effectively starts now that episodes are reachable by listeners.
>
