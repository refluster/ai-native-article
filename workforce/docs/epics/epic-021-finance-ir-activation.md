...
- **Owner**: silas
- **Created**: 2026-07-07
- **Implemented by**: [#524](https://github.com/refluster/ai-native-article/pull/524) (§B.1 idle-talent detector — Story 4 [#458](https://github.com/refluster/ai-native-article/issues/458), first of three halves)
...
> **Status reconciliation (2026-08-05, Nadia — backlog-reconcile). `Accepted (2026-07-08)` → `In-progress (2026-08-04)`.** Bucket: **incidentally done** at the lifecycle level — the status lagged reality, and it lagged it by five days. The epics-README lifecycle exits `Accepted` "once the first implementation PR is **open**"; that condition tripped **2026-07-31T05:46:55Z** when [#524](https://github.com/refluster/ai-native-article/pull/524) opened against Story 4, and the four passes since (08-01, 08-03, 08-04 and their carried notes) each re-read the row as `Accepted` because they scored the gate on *merges*, not on the open-PR wording the lifecycle actually uses. The flip is now beyond argument: **#524 merged `2026-08-04T20:55:46Z` (`e29ca1c`)** and §B.1 is live on `main` — `IDLE_WINDOW_DAYS = 30` / `classifyIdleAgent` / `detectIdleAgents` / `PerfIdleRow` in [`workforce/lambdas/shared/performance.ts`](../../lambdas/shared/performance.ts) (`:178`, `:209`, `:349`, `:370`), the sweep riding the existing daily walk and writing `PERF#workforce` `sk: "IDLE"` in [`workforce/lambdas/performance-reducer/handler.ts`](../../lambdas/performance-reducer/handler.ts) (`:168`, `:286`), the commons class declared on the skill and generated into `COMMONS_SKILLS` by `build-skill-registry.mjs` (set on `feed-post` + `daily-research`), and `validate-skills` rule **J8** guarding it. A second implementation PR — [#531](https://github.com/refluster/ai-native-article/pull/531), Story 2's budget-runway-review Cadence ([#456](https://github.com/refluster/ai-native-article/issues/456)) — has been open since 2026-08-02. Monotonic forward flip; date stamped to the documented go-live (the merge), not to this reconciliation.
>
> **Not `Implemented`, and the remaining gates are named rather than assumed.** #524 is explicitly `Refs #458`, not `Closes` — two of Story 4's three halves are unbuilt: the **weekly-digest rendering** of the idle list (the row exists; nothing reads it) and the **§B.2 hire-time rule** (hiring-playbook amendment + seed-bundle `speculative` field + round-doc lint), which the acceptance criteria themselves mark authority **B** — "drafted for explicit operator accept/reject, never self-merged." Stories 1 ([#455](https://github.com/refluster/ai-native-article/issues/455)) and 3 ([#457](https://github.com/refluster/ai-native-article/issues/457)) have no PR. **Residue with no tracking issue:** #524 flagged one acceptance criterion it could not honour — the digest annotation `"bound, pending enable, N days"` is underivable because no per-binding enable/created timestamp exists on `AgentBinding` or `AGENT#{slug}/META`. That is a schema change, not a rendering detail, and it is the kind of carved-out gate that goes silent if nobody writes it down. Proposed for filing in this pass's PR (issue creation is operator-gated).
