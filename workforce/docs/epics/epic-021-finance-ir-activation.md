...
- **Owner**: silas
- **Created**: 2026-07-07
- **Implemented by**: [#524](https://github.com/refluster/ai-native-article/pull/524) (§B.1, idle-talent detector in the `performance-reducer` walk — merged 2026-08-04), [#531](https://github.com/refluster/ai-native-article/pull/531) (§A.2, `budget-runway-review` Cadence, `Closes #456` — merged 2026-08-05)
...
> **Status reconciliation (2026-09-06, Nadia — backlog-reconcile). `Accepted (2026-07-08)` → `In-progress (2026-08-05)`.** Bucket: **incidentally done at the lifecycle level** — a month-old miss, not a new event. The lifecycle's own exit criterion ("Accepted → In-progress once the first implementation PR is open") tripped on **2026-07-31** when [#524](https://github.com/refluster/ai-native-article/pull/524) opened, and was exceeded again — on the stronger "merged" bar this index has used for every other incidentally-done flip — when [#524](https://github.com/refluster/ai-native-article/pull/524) merged **2026-08-04T20:55:46Z** (§B.1, the idle-talent detector: `IDLE_WINDOW_DAYS`/`classifyIdleAgent`/`detectIdleAgents` in `workforce/lambdas/shared/performance.ts`, sweep writing `PERF#workforce`/`sk:"IDLE"`) and [#531](https://github.com/refluster/ai-native-article/pull/531) merged **2026-08-05T04:50:21Z** (§A.2, the `budget-runway-review` Cadence, `Closes #456` — issue #456 confirmed closed by that merge). **This was already found once and never applied**: the 2026-08-05 backlog-reconcile pass itself proposed exactly this flip in its own draft PR ([#545](https://github.com/refluster/ai-native-article/pull/545)), citing the same two PRs — but #545 was never merged, sat in the operator queue for a month while six more reconciliation passes fired, and none of them re-derived the flip because each pass scoped its audit to the delta since its own predecessor's *merged* base, not to the backlog of the predecessor's *un-merged* proposals. (The Epic-020 half of that same #545 finding *did* get re-applied fresh, by the 2026-08-14 pass via #588 — "on stronger evidence than the last attempt to make this exact flip" — but nothing carried the Epic-021 half forward until now.) Epic-021 still has real open gates: §A.1 (investor letter, #455), §A.3 (fundraising decision-frame, #457), and §A.2/§B.1's own digest-rendering + hire-time-rule follow-ups (#566, with an open-but-stale implementation PR #565) — so this is `In-progress`, not `Implemented`. Mechanical, evidenced, forward-only flip; no Obsoleted/Rejected reclassification, so nothing here needs operator sign-off. Superseded PR #545 closed as redundant with this pass's PR (same dedup precedent as #645→#647, 2026-09-04).
