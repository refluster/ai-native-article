...
| **6c — Credential-leak test** | An integration/contract test asserting `GET /projects/{id}/credentials` and any credential-listing path never returns a secret value, per Epic-010 §10's own invariant. `CredentialVault.test.tsx` covers component render/mask behaviour only; it does not assert the backend contract. | No — this is the security-shaped QA workstream Ren separated out (2026-08-07), not a design call. | none (can ship independently of 6a) |
...
> **Status reconciliation (2026-09-16, Nadia — backlog-reconcile).** 6b shipped: [#743](https://github.com/refluster/ai-native-article/pull/743) converted the free-text `project_id` field to a `<select>` defaulting to `self/{slug}`, auto-closing [#736](https://github.com/refluster/ai-native-article/issues/736) via its own `Closes #736`. Re-verified against `main` (2026-09-16): 6a's member editor and 6c's dedicated credential-leak backend contract test still do not exist (`credential-injector-tests.ts` tests the injector, not the `GET /projects/{id}/credentials` listing contract #737 asks for); 6d remains blocked on both. Epic-010's own top-level Status stays `In-progress` — three of four Story-6 slices are still open, plus the epic's other unclosed DoD gates. Issue diff: 0 closed / 0 rewritten / 0 filed (GitHub's own auto-close already handled #736; no mutation needed from this pass).
