...
- **Q3. Embedding model lock-in.** `voyage-3-lite` is the v1 choice on cost grounds, but vectors are not free to re-embed. The §9 storage shape writes `embedding_model_id` and `embedding_dim` next to each vector on the `EXEC` row so re-embedding on a model change is a DDB scan + write, not a guess. Resolved.
