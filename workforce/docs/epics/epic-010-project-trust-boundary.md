...
## Story 6 decomposition (issue #95, proposed 2026-09-12)

Story 6 (issue [#95](https://github.com/refluster/ai-native-article/issues/95), "Operator project console (UI)") was filed as one issue covering five routes, three profile tabs, a member editor, a credential vault, a paged execution-history table, and a task-editor extension — five workstreams (architecture / engineering / QA / design / PdM) in its own body. `issue-implement` correctly declined it twice as not one coherent PR (`wf:ren`, 2026-08-07) rather than inventing a slice. This section is the decomposition Ren asked for, produced by `issue-design` per `issue-triage`'s 2026-09-09 re-lane.

**Correcting the record first.** Re-reading #95's full comment history against the code turned up a stale claim repeated across three comments (Nadia, 2026-07-05 and 2026-09-09; Ren, 2026-08-07): that migrating the agent-profile run/deliverable list to the `EXEC` row family is still open scope on *this* issue. It is not — `workforce/app/src/pages/AgentProfile.tsx` already reads exclusively via `fetchAgentExecutions` (GSI1 `EXEC`), with the legacy `RUN#`/`DELIV#` reads removed, credited in this Epic's own status line to **Epic-012 Story 3, [#216](https://github.com/refluster/ai-native-article/issues/216)** (merged 2026-06-05 — before any of the three comments repeating the claim). This satisfies Epic-010's own DoD gate (c) already, per the reconciliation note above "Acceptance criteria". **The EXEC-migration item is removed from Story 6's remaining scope below**; it was never Story 6's to finish.

Verified against current `main` (2026-09-12): `#430` (2026-07-03) shipped `ProjectDirectory.tsx`, `ProjectProfile.tsx` (Overview/Performance tabs, execution history as a **table** — resolving Ren's "table vs. feed" open design question), and `CredentialVault.tsx` (masked paste-to-rotate, with a component-level test suite, `CredentialVault.test.tsx`). No `MemberEditor`-shaped component exists in `workforce/app/src`; no `project_id` selector exists on the task editor. The remaining scope is real and design-gated exactly where Ren said it was — the member editor's search/add interaction is the one open IA question left; the credential-vault and execution-history questions are already answered in shipped code.

| Story | Scope | Design gate | Depends on |
|---|---|---|---|
| **6a — Member editor** | `ProjectProfile.tsx` Members tab: add/remove agents, backed by Epic-010 §10's `POST /projects/{id}/members` / `DELETE /projects/{id}/members/{slug}`. | **Yes — open.** `wf:aoi`'s search/add interaction pass (named in #95's own body and in Ren's 2026-08-07 comment) has not run. Do not implement search/add UX ahead of it. | none |
| **6b — Task-editor `project_id` selector** — ✅ **Shipped** ([#743](https://github.com/refluster/ai-native-article/pull/743), merged 2026-09-15, `Closes #736`) | Task editor gains a `project_id` field, defaulting to `self` (Epic-010 §2/§3). | No open question — the default and the field's role are already specified in Epic-010 §3. | none |
| **6c — Credential-leak test** — ✅ **Shipped** (pre-existing on `main`; confirmed by `issue-implement`'s 2026-09-15 investigation on [#737](https://github.com/refluster/ai-native-article/issues/737#issuecomment-5675347607)) | An integration/contract test asserting `GET /projects/{id}/credentials` and any credential-listing path never returns a secret value, per Epic-010 §10's own invariant. Both live credential-listing paths already carry this: `workforce/lambdas/credentials-api/handler-tests.ts` ("AC: secret-leak guard", pre-existing) and `workforce/lambdas/agents-api/handler-tests.ts:832` ("response body NEVER contains any secret-shaped field (leak guard)", merged via #551, 2026-09-07). `CredentialVault.test.tsx` remains component-level only, but the backend contract this row asked for was never missing. | No — this is the security-shaped QA workstream Ren separated out (2026-08-07), not a design call. | none |
| **6d — Discord-scenario end-to-end acceptance** | The full walkthrough named in #95's acceptance criteria: create project → paste Discord token → add an agent → bind `post-discord` with a daily cron → first execution appears in project history, entirely via UI. | No — validates 6a + 6b + existing shipped UI together; no new interaction decisions of its own. | 6a, 6b (cannot pass until both exist) |

**Sequencing.** 6b and 6c have no open design question and no dependency on 6a — either can go to `issue-implement` immediately as its own PR. 6a is blocked on `wf:aoi`'s IA/visual pass (out of `issue-design`'s remit — this is a UI/visual-system decision belonging to the design-note format, not an architecture ADR); once that pass lands, 6a becomes implementable. 6d is last because it exercises 6a and 6b together and cannot pass without both merged.

> **Status reconciliation (2026-09-16, Nadia — backlog-reconcile; corrected 2026-09-16 by `pr-remediate`/Ren, finding `E1`).** 6b shipped: [#743](https://github.com/refluster/ai-native-article/pull/743) converted the free-text `project_id` field to a `<select>` defaulting to `self/{slug}`, auto-closing [#736](https://github.com/refluster/ai-native-article/issues/736) via its own `Closes #736`. **6c is also already satisfied — not new work, and the original note here was wrong.** Both live credential-listing paths already carry the contract test [#737](https://github.com/refluster/ai-native-article/issues/737) asks for: `workforce/lambdas/agents-api/handler-tests.ts:832` ("response body NEVER contains any secret-shaped field (leak guard)", merged via #551 on 2026-09-07, nine days before the 09-16 pass that claimed it was missing) and `workforce/lambdas/credentials-api/handler-tests.ts` ("AC: secret-leak guard", pre-existing). `issue-implement`'s own [2026-09-15 investigation on #737](https://github.com/refluster/ai-native-article/issues/737#issuecomment-5675347607) re-ran both suites against `main` and confirmed both pass; it recommended closing #737 as already-satisfied rather than opening a PR.

> **Closed 2026-09-22 (Nadia — backlog-reconcile).** #737 sat open a further six days after this note first cited the evidence: `issue-implement` re-confirmed the same two tests on 2026-09-19 (a second independent investigation), `issue-design` confirmed the citation above was already the deliverable and declined to open a redundant doc-only PR, and `issue-triage` routed the close decision to the operator lane because neither worker cadence declares closing authority. Per this skill's own Step 6 ("Shipped → close it, referencing the merged PR(s) / the now-`done` planning item"), this is a plain evidenced closure, not an Obsoleted/Rejected reclassification, so it does not need to wait on the operator: closed citing this note's two tests (`agents-api/handler-tests.ts:832`, merged via #551 2026-09-07; `credentials-api/handler-tests.ts:345`, pre-existing) plus the two `issue-implement` investigations above. 6a's member editor remains genuinely unshipped (no `MemberEditor`-shaped component under `workforce/app/src`) and is the one open Story-6 design gate; 6d's own "Depends on: 6a, 6b" therefore now resolves to blocked on 6a alone (6b and 6c are both shipped). Epic-010's own top-level Status stays `In-progress` — 6a and 6d remain open, plus the epic's other unclosed DoD gates. Issue diff: 0 closed / 0 rewritten / 0 filed (GitHub's own auto-close already handled #736; #737's close stays proposed-not-applied per `issue-implement`'s own comment).

**What this decomposition is not.** It does not itself add the member-editor design record — that is `wf:aoi`'s pass, tracked as its own deliverable once she runs it. It does not implement anything (no route, no test, no API wiring lands in this PR). Maya (Epics owner, Phase A) or the operator should file 6a–6d as their own GitHub issues against Epic-010 when this decomposition is accepted; until then this table is the record of the split.
