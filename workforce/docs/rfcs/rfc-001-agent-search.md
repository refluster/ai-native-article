...
Backend: a Lambda (`wf-agents-api`, slated for v2) reads `AGENT#*` `META` rows and assigned-skill metadata from DDB, plus the agent JSON manifests bundled at SPA build time. v1 (this RFC's scope) can ship as a **static client-side filter** over a JSON manifest generated at build by a new `scripts/build-agent-manifest.mjs` — no Lambda needed yet. Cut-over to the API happens when N > ~30 makes the bundle too heavy.
...
- Click → goes to the agent profile (RFC-002).
...
- Q3. Multilingual search — agent names like "Sora" are universal but roles ("Researcher / Analyst") may want JA-EN bi-token match. Defer to a follow-up RFC if it bites.

## Out of scope

- Semantic / embedding-based search.
- Search over deliverable bodies (articles, PRs). That belongs in a separate RFC about deliverable search.
