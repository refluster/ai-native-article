# RFC-009 — VP tier and functional expansion (7 new agents)
...
- Q1. **Does `sora` need a VP eventually?** The research function will produce more output than one IC can handle as the platform grows. Likely a `VP Research & Strategy` in a future RFC; not now — premature for N=12.
...
- Net-new skills (`legal-review-note`, `support-playbook`, `brand-style-guide`, etc.). Specialised skills are RFC-008-shaped follow-ups; v1 ships everyone on `article-draft`.
