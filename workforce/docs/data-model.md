...
| `pk` | `sk` | Purpose | Key attributes |
|---|---|---|---|
| `PROJECT#{project_id}` | `META` | Project descriptor | `project_id`, `status` ∈ `{active, archived}`, `owner_agent` (slug or `_operator`), `created_at`, `archived_at?`. **Project variables** (non-confidential config seeded from `workforce/projects/{id}/project.json` by `seed-projects.mjs`, surfaced read-only in the console; canonical edits go through `project.json` + seed per §10, not the PATCH API): `name`, `github_owner?` / `github_repo?` (the standard **target-repo** attribute, flattened from `project.json:github.{owner,repo}` — e.g. `refluster/project-ind`, `PSVL/asp-cloud`; read by the pr-autopilot / pr-review path per [governance.md](governance.md) R-N9/R-N10 to resolve PR URLs). The matching PAT is a *credential*, not a variable — it lives in Secrets Manager under `wf/projects/{id}/github.token` (the github-repo-var vs. github-repo-secret split). The seed additionally writes `credential_types[]`, `governance_docs[]`, `note?`, `identity_hash`, `updated_at`. |
| `PROJECT#{project_id}` | `MEMBER#{agent_slug}` | Project membership row (**roster metadata only — gates nothing**) | `project_id`, `agent_slug`, `joined_at`, `revoked_at?`. `removeMember` is a **soft delete** — it writes `revoked_at` rather than dropping the row, so the audit question "was X a member of Y on date Z" can be reconstructed. `isMember`/`members` filter on `revoked_at === undefined`. **No access gate:** the `appendExecution` write-gate was removed 2026-06-08 and the `listExecutions`/`recall` read-gate 2026-06-10 (owner directive; C-3) — every registered agent participates in every project, so this row is an audit/roster record, not an authorization check. |
