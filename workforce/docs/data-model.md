...
|---|---|---|---|
| `AGENT#{slug}` | `META` | Agent identity/config — **authoritative store** ([ADR-0007](adr/adr-0007-agent-config-single-source.md); formerly a mirror of `workforce/agents/{slug}/`). Written only via agents-api (`POST /agents` create, `PATCH` mutate) | Identity: `slug`, `first_name`, `last_name`, `residence`, `role`, `model`, `prompt_version`, `system_prompt` (inline persona prompt, the former `system.md` body), `budget_monthly_usd_default`, `default_project`, `streams[]`, `bindings[]` (each: `{skill, executor, trigger, routine_spec?, project_id?, config?, workflow?, note?}` — see [runbooks/bindings.md](runbooks/bindings.md)), `created_at`. Operational: `budget_monthly_usd_override?`, `paused`, `archived`, `last_run_at?`, `last_run_status?`. Computed: `runs_this_month`, `cost_this_month_usd`, `deliv_count_total`. Plumbing: `updated_at`, `identity_hash` (seed-era optimistic-concurrency token; retires at ADR-0007 step 6), `config_owner?` (`"ddb"` once API-written — the transitional seed-skip marker, retires at step 6) |
| `AGENT#{slug}` | `AUDIT#{iso-ts}#{nonce}` | Config-mutation audit trail (ADR-0007 Decision §4) — append-only, written by agents-api on **every** accepted mutation; the git-history replacement | `slug`, `at` (ISO), `actor` (IAM principal ARN), `source` = `agents-api`, `kind` ∈ `{create, identity, operational}`, `changes[]` (`{field, before, after}`; string values > 1 KB are stored as `{truncated: true, length, sha256, head}` digests — the authoritative text lives on `META`). Read by `GET /agents/{slug}/audit` (newest-first) and the weekly `wf-config-digest` Lambda (Decision §5) |
...
| `pk` | `sk` | Purpose | Key attributes |
|---|---|---|---|
| `SKILL#{name}` | `META` | Skill identity/config. `name` is the **immutable slug** (DDB key, binding reference, EXEC `skill_name`, GSI2 partition — never changes; [ADR-0017](adr/adr-0017-skill-lifecycle-api.md)); `display_name?` is the renameable human label. **Judgment-side fields are authoritative here** ([ADR-0008](adr/adr-0008-skill-config-single-source.md)): `body` (the SKILL.md judgment text), `description`, `display_name?`, `version`, `status` ∈ `{active, stale, deprecated, archived}` (`archived` = soft delete: hidden from the default list, rejected as a NEW binding target, history intact — ADR-0017), `owners[]`, `cost_class`, `improvement_agent`. Created by `wf-seed-skills` from a new `workforce/skills/{name}/` folder, re-synced from git on a **version gate** ([ADR-0018](adr/adr-0018-skill-body-version-gated-sync.md): an existing row's judgment fields re-sync only when git `meta.json:version` is strictly newer than the row's, audited like a PATCH — narrowing ADR-0008's create-only seed), **or created via `POST /skills` for judgment-only skills** (ADR-0017; `identity_hash: "api"`), mutated via agents-api `PATCH /skills/{name}`. Code-side artefacts (write-scripts, `requires[]` → `skill-registry-generated.ts`, `archetype`, `deliverable`) stay git-owned. Per-skill run ledger read: `GET /skills/{name}/executions` (GSI2). Operational: `improvement_agent_override?`. Computed: `invocations_this_month`, `last_invoked_at?` |
...
| `pk` | `sk` | Purpose | Key attributes |
|---|---|---|---|
| `PROJECT#{project_id}` | `META` | Project descriptor | `project_id` (the immutable slug — DDB key, URL segment, Secrets Manager prefix), `status` ∈ `{active, archived}`, `owner_agent` (slug or `_operator`), `created_at`, `archived_at?`. **Display name** `name?` is **decoupled from the slug and API-writable** (`PATCH /projects/{id}`, AWS_IAM): a rename never moves the id, and the seed treats `name` as create-only on existing rows so a PATCHed name survives re-seeds. Other **project variables** (non-confidential config seeded from `workforce/projects/{id}/project.json` by `seed-projects.mjs`, surfaced read-only in the console; canonical edits go through `project.json` + seed per §10): `github_owner?` / `github_repo?` (the standard **target-repo** attribute, flattened from `project.json:github.{owner,repo}` — e.g. `refluster/project-ind`, `PSVL/asp-cloud`; read by the pr-autopilot path per [governance.md](governance.md) R-N9/R-N10 to resolve PR URLs). The matching PAT is a *credential*, not a variable — it lives in Secrets Manager under `wf/projects/{id}/github.token` (the github-repo-var vs. github-repo-secret split). The seed additionally writes `credential_types[]`, `governance_docs[]`, `note?`, `identity_hash`, `updated_at`. |
| `PROJECT#{project_id}` | `MEMBER#{agent_slug}` | **RETIRED (2026-07-03)** — historical membership rows | The membership concept was removed (operator direction): every registered agent participates in every project; only `owner_agent` remains as a responsibility pointer. The concept had already been reduced to "roster metadata that gates nothing" (the `appendExecution` write-gate removed 2026-06-08, the `listExecutions`/`recall` read-gate 2026-06-10). Existing rows (`project_id`, `agent_slug`, `joined_at`, `revoked_at?`) remain in the table as inert audit data — nothing reads or writes them; the helpers, routes (`GET /projects/{id}/members`, `GET /agents/{slug}/projects`), and seed writes are gone. |
| `PROJECT#{project_id}` | `EXEC#{ulid}` | Execution ledger row | `project_id`, `agent_slug`, `skill_name`, `skill_version`, `started_at`, `ended_at`, `status` ∈ `{ok, throw, skipped, failed_artefact_redaction}`, `used_credential_types[]`, `inputs_hash?`, `artifact_ref?` (`{uri, content_hash, content_type, size_bytes, summary ≤512c}`), `summary?` (≤512c free-text business summary of the engagement — what the unit of work accomplished; **distinct from `artifact_ref.summary`**, which previews a produced *file*: an engagement may carry a summary with no file artifact, e.g. `pr-review`. Set by the `POST /agents/{slug}/engagements` route; the portfolio / RUNS·DELIVERABLES UI renders this, falling back to `artifact_ref.summary` then `""`. Pre-2026-06-13 rows have no attribute), `error?`, `execution_surface?` ∈ `{lambda, client, ccr}` (absent → `lambda` by convention; the `POST /agents/{slug}/engagements` route writes `client` for R-N1(b) external audit POST-backs and `ccr` for the per-task write-back from a CCR routine run — ADR-0005; legacy rows have no attribute). GSI1 (`gsi1pk=AGENT#{agent_slug}, gsi1sk=started_at`) for agent-scoped recall; GSI2 (`gsi2pk=SKILL#{skill_name}, gsi2sk=started_at`) for skill-utilisation queries. |
