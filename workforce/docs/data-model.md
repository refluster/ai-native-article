...
| `pk` | `sk` | Purpose | Key attributes |
|---|---|---|---|
| `SKILL#{name}` | `META` | Skill identity/config. `name` is the **immutable slug** (DDB key, binding reference, EXEC `skill_name`, GSI2 partition — never changes; [ADR-0017](adr/adr-0017-skill-lifecycle-api.md)); `display_name?` is the renameable human label. **Judgment-side fields are authoritative here** ([ADR-0008](adr/adr-0008-skill-config-single-source.md)): `body` (the SKILL.md judgment text), `description`, `display_name?`, `version`, `status` ∈ `{active, stale, deprecated, archived}` (`archived` = soft delete: hidden from the default list, rejected as a NEW binding target, history intact — ADR-0017), `owners[]`, `cost_class`, `improvement_agent`. Created by `wf-seed-skills` from a new `workforce/skills/{name}/` folder (CREATE-ONLY — never overwrites) **or via `POST /skills` for judgment-only skills** (ADR-0017; `identity_hash: "api"`), mutated only via agents-api `PATCH /skills/{name}`. Code-side artefacts (write-scripts, `requires[]` → `skill-registry-generated.ts`, `archetype`, `deliverable`) stay git-owned. Per-skill run ledger read: `GET /skills/{name}/executions` (GSI2). Operational: `improvement_agent_override?`. Computed: `invocations_this_month`, `last_invoked_at?` |
