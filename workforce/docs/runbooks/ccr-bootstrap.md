...
> **Status (2026-05-27)**: the **label-driven state machine** described below was retired by the simplified dev process in [dev-process.md](dev-process.md). The repo now has **0 labels** — the agent holding `pr-route` routes via PR comments per [pr-route](../routines/pr-route.md), and the generic [pr-review](../routines/pr-review.md) skill is invoked manually under the invoking persona's binding `config` (or via a future CCR API trigger), not by labels.
>
