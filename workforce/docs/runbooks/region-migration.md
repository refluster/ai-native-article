...
- **The API base URL (`VITE_WORKFORCE_AGENTS_API_BASE` + the `discord-digest` constant) is region-coupled — unless the custom domain is in place.** The raw `https://{api-id}.execute-api.{region}.amazonaws.com/prod` URL bakes in both the region and the (region-specific) API id, so a migration forces a coordinated update of the SPA secret and every script that carries the base URL. [ADR-0004](../adr/adr-0004-workforce-api-custom-domain.md) removes this: once the `wf-api-domain-{stage}` stack ([`../../infra/sam-api-domain/README.md`](../../infra/sam-api-domain/README.md)) serves the API under `workforce-api.kohuehara.xyz`, a migration only re-points one Cloudflare CNAME at the new region's regional API GW target — the SPA secret and the skill constant stay put. If the custom domain is **not** yet rolled out, treat the base-URL cutover as an explicit migration step.
