...
**Review is persona-agnostic; persona is in the binding.** There is no separate `pr-review` skill — reviewing is the middle leg of [`pr-autopilot`](../../skills/pr-autopilot/SKILL.md), whose Step 4 carries the canonical reviewer-lens contract (read PR + Story → post inline + summary → finding-IDs → green marker → bias disclosure → sign-off). The persona-specific lens (Dario's R-N\* / Ren's TS idiom / Aoi's design system) lives on each agent's `AGENT#{slug}` skill-judgment `config`. The router applies each nominated persona's lens inline and posts it as that persona — the binding's `config.lens_name`, `config.values`, `config.checklist_sections`, and `config.bias_disclosure_template` decide what they look for. See [bindings.md](bindings.md) for the config shape.
