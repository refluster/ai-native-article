...
- **R-N10. Delegated external-merge (the one bounded exception to "agents never merge").** A workforce skill MAY merge a Pull Request on an **external** project's repo — overriding the §5 "agents never merge" default and the R-N9 author-only posture — **only when every one of the following holds**; otherwise it MUST escalate (post a review comment + hand off, or file a tracking issue):
  1. **Delegation exists.** The target project's *own* governance has explicitly granted autonomous-merge authority to the workforce agent for a named PR class (e.g. `PSVL/asp-cloud` → `docs/adr_autopilot_pr_merge.md`, the "Autopilot PR" lane). The grant lives in the *target repo's* statute; the workforce never self-asserts it. This is the W-5 / R-N9 reconciliation: those rules forbid the workforce *taking* merge authority the maintainer never gave — R-N10 permits a merge *only where the maintainer has given it.*
  2. **Eligibility predicate passes (machine-checkable).** The PR meets the target project's published predicate. For the asp-cloud Autopilot lane: a clean Dependabot **security-update** PR, lockfile-only (touches no L1-binding path), semver-patch or minor-on-≥1.0 (no major / `0.x`-minor-crossing), **all required checks green**, reachability/severity-bounded, no human `CHANGES_REQUESTED`. The skill's bundled write-script **re-verifies the predicate server-side and fails closed.**
  3. **Kill-switches armed.** Both the target repo's switch (e.g. `AUTOPILOT_PR=on`) and the workforce-side per-binding switch are on.
  4. **Audited & legible.** The merge posts a CVE/GHSA-cited triage comment, the squash commit references the advisory id, and the fire self-records an engagement (R-N1 audit ledger). The mechanical marker is the deliverable type **`external-pr-merge`** (registered by this amendment in `skill-meta.schema.json` + `skill.ts:DeliverableType`): a skill that performs delegated merges MUST declare it, and `external-pr` skills still cannot merge.
  This is the realisation of the 2026-06-16 Autopilot direction; it is the **only** path by which a workforce agent merges anything, and the rule is itself Zone A (this amendment). **Consumers (one shared engine).** Two skills perform delegated merges and MUST both declare `external-pr-merge`: the `dependabot-triage` Cadence (the no-review fast path) and `pr-autopilot` cycle-2 verdict mode (merge only on a 🟢 verdict for a safe-class PR). Both call the **single** fail-closed engine `workforce/skills/pr-autopilot/pr-merge.mjs`, which re-verifies clauses 1–3 server-side; there is one merge implementation, not per-skill copies. Routing/review under `pr-autopilot`/`pr-review` generalise to **all** PRs, but the *merge* never widens past this predicate without a superseding R-N10 amendment.
...
| Merge any PR (including agent-authored ones). | **B** | Agents never merge — **except** a *delegated external-merge* under **R-N10** (an autopilot-eligible PR on an external repo whose own statute has granted the workforce merge authority, predicate-passing, kill-switches armed). Every other merge escalates. Inherits AGENTS.md R-6. |
| Perform a delegated external-merge under **R-N10**. | **A** (autonomous, bounded) | Only when all four R-N10 clauses hold (delegation in target statute, predicate passes server-side, kill-switches armed, audited via `external-pr-merge` + CVE comment + engagement). Any failed clause → escalate (file issue). |
