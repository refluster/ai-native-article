...
- **R-N10. Delegated external-merge (the one bounded exception to "agents never merge").** A workforce skill MAY merge a Pull Request on an **external** project's repo — overriding the §5 "agents never merge" default and the R-N9 author-only posture — **only when every one of the following holds**; otherwise it MUST escalate (post a review comment + hand off, or file a tracking issue):
  ...
  This is the realisation of the 2026-06-16 Autopilot direction; it is the **only** path by which a workforce agent merges anything, and the rule is itself Zone A (this amendment). **Consumers (one shared engine).** Two skills perform delegated merges and MUST both declare `external-pr-merge`: the `dependabot-triage` Cadence (the no-review fast path) and `pr-route` cycle-2 verdict mode (merge only on a 🟢 verdict for a safe-class PR). Both call the **single** fail-closed engine `workforce/skills/pr-route/pr-merge.mjs`, which re-verifies clauses 1–3 server-side; there is one merge implementation, not per-skill copies. Routing/review under `pr-route`/`pr-review` generalise to **all** PRs, but the *merge* never widens past this predicate without a superseding R-N10 amendment.
