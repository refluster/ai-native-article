...
- **W-3 Cost ceiling.** A monthly token budget per agent is enforced at the LLM call site (throw on overrun). A CloudWatch Billing Alarm covers the deployment as a whole. **Current cap: USD 275/month combined.** Raising the cap is a Zone A change; every raise is one row in the amendment table below (per-hire rationale lives in the round docs under [`hires/`](hires/)).
  ...
  | 2026-07-08 | 250 → 275 | India Energy Market Research Desk (Anjali / Rohan / Sneha / Sofia) |
...
- **R-N1. Execution surfaces (declared).** Agent reasoning runs on AWS Lambda by default. Documented exceptions:
  ...
  - **(c) Podcast production & distribution surface (Epic-017, [ADR-0016](adr/adr-0016-podcast-production-surface.md)).** The deterministic `wf-podcast` Lambda runs on the **default** Lambda surface (it performs no agent reasoning — synthesis and feed assembly are deterministic), and the `podcast-script` Cadence runs on the existing exception (a) CC-routine surface. What is **new** and hereby declared: the Lambda's **Amazon Polly** call (a new AWS service in the SAM template — the §5 "new AWS service" B-authority row) and the **public S3 RSS/MP3 egress** (the podcast feed enclosure is public by nature, so Spotify's crawler can fetch it — served from the existing `wf` bucket's `podcast/` prefix via the bucket's CDN, never from a public bucket ACL). No new *reasoning* execution surface is introduced, so the "no other surfaces" clause below is unaffected.
...
- **R-N4. Bindings are the unified scheduler declaration.** Every scheduled or event-driven workforce execution is declared in the agent's `AGENT#{slug}/META.bindings[]` (written via agents-api per [ADR-0007](adr/adr-0007-agent-config-single-source.md); formerly `workforce/agents/{slug}/agent.json:bindings[]`). A binding specifies `{skill, executor, trigger}`:
  ...
  - The agents-api write-time validator (`workforce/lambdas/shared/agent-config.ts`) asserts every binding's structural contract — executor/scheduler allowlists, skill **existence** against the `SKILL#` rows, the CCR-batch `project_id` requirement — synchronously on every write (ADR-0007; formerly the `validate-agent-json.mjs` CI lint, which retires with the git tree at step 6). Binding is **not** gated on skill ownership: any agent may bind any existing skill ([adr-0012](adr/adr-0012-decouple-binding-from-ownership.md)). `owners[]` retains its authorship/Rule-11/improvement meaning only.
