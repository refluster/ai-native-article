...
4. **Assemble the recall packet** per the skill's contract. The skill body describes what context it wants (recent EXEC rows / memory chunks / TASK queue / repo state / etc.). The wf-agents-api base URL is the same one carried by each skill's write script as a constant (e.g. `DEFAULT_API_URL` in `workforce/skills/feed-post/post-feed.mjs`) — a static constant, so the orchestrator does not need to inject a `task.agents_api_url` field for this. Example shape:
   ...
   **Structured (semantic) recall — the same path `messaging-reply` uses in-process, exposed here as a GET.**
   Epic-012 Story 1 built `buildRecallBlock()` / `recall()` (`workforce/lambdas/shared/{recall,recall-prompt}.ts`) and wired it into the board Q&A reply path, but this runner's step 4 never called the equivalent HTTP surface — it only described ad hoc GETs with no ranking, no char cap and no fail-loud truncation guard (#761). The HTTP surface already exists (`GET /agents/{slug}/recall?q=&k=`, `workforce/lambdas/agents-api/handler.ts`), so closing the gap needs no new Lambda code — only this runner actually calling it:

   - Read `skill.recall_k` from the layer-4 `GET /skills/{skill}` response fetched in step 2 (absent → default **3**, the same `RECALL_K_DEFAULT` `recall-prompt.ts` uses). `recall_k: 0` means the skill opts out — skip this sub-step entirely.
   - Build the query the same way `buildRecallQuery()` does: `"<skill> — <one-line task brief, if the skill has one> — <project_id>"` (join only the parts that exist).
   - Call `GET <wf-agents-api-base>/agents/{agent_slug}/recall?q=<query>&k=<recall_k>&project=<project_id>` — caller-scoped by construction (Epic-012 Q2: an agent only ever recalls its own ledger), so no further filtering is needed.
   - Render the returned `items` into the working prompt under a `## Relevant past work (recalled)` heading, one line per item as `- [<started_at date> · <skill_name> · <status>] <summary, falling back to artifact_ref.summary, then error, then "(no summary)">` — mirroring `renderRecallLine()` — **within a 1,500-character budget** (`RECALL_BLOCK_CHAR_CAP`), keeping the highest-scored (most relevant) items first. On overflow, keep the prefix that fits and append a visible `(N less-relevant match(es) omitted for prompt budget)` marker — never a silent trim. If a single item's line alone exceeds the cap, that is a misconfiguration, not a transient: fail this task loudly (C-4) rather than truncate it silently.
   - Fail-soft on retrieval only: a non-2xx from `/recall`, a timeout, or an empty `items` list means an empty recall block, not a failed task — recall is an enhancement, never a run-blocker, matching `buildRecallBlock()`'s own try/catch. The budget/single-entry-overflow check above is the one loud exception, exactly as in `renderRecallBlock()`.
