...
- **Status**: Accepted (2026-09-02 — ratified by the merged implementation, per this ADR's own clause [operator ratifies by merging the implementation PR]; status reconciled by `backlog-reconcile`, flagged for operator sign-off)
- **Date**: 2026-08-30
...
> **Status reconciliation (2026-09-02, Nadia — `backlog-reconcile`). `Proposed` → `Accepted`; ⚠ this encodes a ratification and needs operator sign-off via the merge, same shape as ADR-0025 (2026-08-12 pass).** This ADR's own status line named the operator's merge of the implementation PR as the ratifying act, and by this pass's audited `HEAD` (`51055e1`) that has happened twice over: **PR #642** ("Epic-025 Phase 1: project Tools surface + the azure.openai credential type," merged 2026-08-31T04:25:48Z) shipped the `azure.openai` credential type across its mirror points and the empty-registry Tools tab; **PR #644** ("Epic-025 Phase 2: tools-api, the Azure OpenAI path, and the first two tools," merged 2026-08-31T18:01:19Z) shipped the `tools-api` Lambda, the registry loader, `shared/llm-azure.ts`, EXEC-row writing, and the first two tools (Problem Finding, User Research) live on it. Epic-025's own file already carries both PR links under "Implemented by" and its Status/index row already read `In-progress (2026-08-31)` — set by the operator directly in-session, not by this cadence — so this flip brings the *ADR* record into agreement with a fact the *Epic* record already stated. **What remains genuinely open, and is not this flip's business:** Phases 3–6 (Task Process, Business Impact Builder, Insight Foundry, EOL) are unbuilt — Epic-025 stays `In-progress`, correctly, and this ADR's ratification covers the *decisions* (Azure OpenAI, the registry shape, the credential type), not the full migration.
>
> **Why this ADR ratifies today and its three 2026-08-30 siblings (ADR-0026/0028/0029) do not, despite each having a merged implementation on the same `HEAD`.** Re-applying the rule this repo's own 2026-08-03 reconciliation established (`workforce/docs/adr/README.md`'s index note): an ADR self-ratifies through this cadence only when its own Status line carries the explicit clause naming "operator ratifies by merging the implementation PR" — this ADR did (added at authoring, PR #641); the other three's Status lines read plain `Proposed`, no clause, so flipping them here would be a design decision this cadence doesn't own — the same restraint the 2026-08-03 note applied to ADR-0022/0023. Their implementations are, in fact, already merged (ADR-0026 by its own authoring PR #637, self-contained; ADR-0028 likewise by #640; ADR-0029 by #643) — noted in [`epics/README.md`](../epics/README.md)'s reconciliation trail for operator attention, not applied here.
