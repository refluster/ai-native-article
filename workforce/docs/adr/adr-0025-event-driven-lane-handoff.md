...
> **Status reconciliation (2026-08-12, Nadia — `backlog-reconcile`). `Proposed` → `Accepted`; ⚠ this encodes a ratification and needs operator sign-off via the merge.** The status line named the operator's merge of the implementation PR as the ratifying act, and that merge has happened: **PR #568** ("event-driven author-lane hand-off (adr-0025) + the missing asp-cloud pr-remediate binding") was opened *and merged* by the operator (`refluster`) directly, 2026-08-11T19:57:46Z, closing #567's related `pr-autopilot` fix the same day. `POST /dispatch`, `workforce.dispatch_token` (`AUTH#DISPATCH` rows), the two chain-point dispatches, and `wire-pr-remediate-ren-asp-cloud.mjs` are all on `main` — PR #568's own checklist confirms `workforce/lambdas` typecheck + 1149 tests green including 16 `dispatch` + 4 `dispatch-token` + 7 `request-dispatch` cases. This mirrors ADR-0018's exact ratification shape (07-30 pass), so the same rule applies: the merge *is* the sign-off event, not a step still owed. **What remains genuinely open, and is not this flip's business:** (a) the end-to-end chain is unverified post-deploy (PR #568 states this itself — `POST /dispatch` and the per-fire token mint don't exist until the data-plane deploy lands); (b) `wire-pr-remediate-ren-asp-cloud.mjs` has not been run — `GET /agents/ren` still shows 5 bindings, no `pr-remediate@asp-cloud`, re-verified 2026-08-12 — filed as [OP-016](../follow-ups.md). Without (b), a dispatch to that binding 404s and the `PSVL/asp-cloud` author lane still has no worker; the event mechanism and the missing-binding bug are two different fixes shipped in one PR, and only the first is complete by this merge.
