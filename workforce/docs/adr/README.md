...
| [0040](adr-0040-per-skill-dispatch-dedup-marker.md) | Per-`(agent, skill)` dispatch marker (`last_dispatch_by_skill` map on the `AGENT#` row) replaces the any-skill `last_run_at` dedup key, which nothing has written since the retired `wf-agent-runner` Lambda | Proposed | [012](../epics/epic-012-agent-experience.md) |
