...
| [0030](adr-0030-escalated-pr-groom-lane.md) | The human lane is groomed, not frozen: a bounded agent keeps an escalated PR mergeable against a moved base — additive conflicts only, with a registry-id collision guard — while the merge decision stays the operator's | Proposed | — |
...
**Numbering note:** 0031 (`adr-0031-spotify-url-automation-and-gate-retirement.md`) and 0032 (`adr-0032-lesson-partition-and-daily-distiller.md`) are drafted in open, unmerged PRs (#693, #695) as of this ADR's own drafting; 0033 was assigned to avoid colliding with either. Whichever of the three merges last should renumber or reconcile per this convention.
