#!/usr/bin/env node
// audience-loop/post.mjs — deterministic write for the "audience-loop" Cadence,
...
// store. No human approval gate. Note: the write lands on the WORKFORCE FEED
// only — this script never sends anything to readers or external channels.
//
// Body is read from a FILE (not an arg) so multi-line / Unicode (Japanese)
// prose can't be mangled by shell quoting. The write CREDENTIAL is injected
// per-fire; the endpoint URL is NOT a secret and is the single source of
// truth right here.
//
// Usage:
...
// overwrites --body-file between our readFileSync and this POST publishes ITS
// prose under OUR slug, and the script still exits 0. That is exactly the
// silent degradation C-4 forbids, and it is not theoretical: the identical
// race published wrong content under 4 agents' slugs on the 2026-08-17 fire
// alone (grid-watch's own sibling `feed-post`/`daily-research` cadences).
...
// improvement, observation}. An experiment step (staged draft or verdict
// report) is beat-noticing on the audience loop — it is ALWAYS an
// observation; the constant lives here (the deterministic layer) so the LLM
// cannot mis-tag it.
