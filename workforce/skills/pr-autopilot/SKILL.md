...
2. **Complete the cycle** — never leave it open. **Escalation ALWAYS carries the `autopilot:needs-human` label — no exceptions.** Every comment that hands a PR to a human MUST (a) embed the hidden marker `<!-- autopilot:needs-human -->` in the verdict body **and** (b) be posted with the `--needs-human` flag. `pr-autopilot-post.mjs` stamps the canonical label from *either* signal (belt-and-suspenders, so a forgotten flag still labels from the marker), so the operator finds the whole queue with `is:open label:autopilot:needs-human`:
   ...
   **The `autopilot:reviewed` companion label (operator directive 2026-06-23).** A 🟢 unanimous-green PR that is handed off **only** because a human gate blocks the autonomous merge (an L0/L1 path, or no R-N10 delegation) is *reviewed and merge-ready* — the operator's only task is the final merge click. Stamp it `autopilot:reviewed` (the `--reviewed` flag and/or the `<!-- autopilot:reviewed -->` body marker — belt-and-suspenders, exactly like `--needs-human`) **in addition to** `autopilot:needs-human`. So `is:open label:autopilot:reviewed` is the merge-ready queue, a strict subset of `is:open label:autopilot:needs-human` that excludes the 🔴 / cycle-capped / non-consensus PRs that still need work. `autopilot:reviewed` is **never** stamped on a non-green hand-off, and never implies a merge — the agent still does not merge an L0/L1 PR.
   ...
   > **Drafts ([adr-0014](../../docs/adr/adr-0014-autopilot-mark-draft-ready-on-merge.md)).** A 🟢 unanimous-green, **non-L0/L1** draft is **not** handed off for the draft flag — the merge engine marks it ready-for-review as the terminal pre-merge step and merges it (see "The merge leg" below). The draft-as-second-gate case survives **only** when the draft *escalates* (it touches **L0/L1**, so a human must merge): there the draft stays a draft, gets `autopilot:reviewed`, and the verdict MUST say **"Still a draft — mark ready, then merge."** so `autopilot:reviewed` never reads as merge-now on something the human must both ready and merge.
...
> **Drafts are merged, not handed off ([adr-0014](../../docs/adr/adr-0014-autopilot-mark-draft-ready-on-merge.md)).** A draft is a *pending*-mergeable state, not a refusal: the engine verifies the real gates (L0/L1, checks, consensus, no conflicts) and, if they all pass, marks the draft **ready-for-review** (GraphQL `markPullRequestReadyForReview`) as the **terminal** pre-merge step, then merges — so a green, non-L0/L1 draft (the workforce opens these by default) flows straight through instead of stalling. This is bounded to the merge path: a draft that *escalates* (L0/L1 / 🔴 / non-consensus) is never un-drafted — it stays a draft for the human (see the Step 5 draft note). Emit the **same** `action:"merge"` decision for an eligible draft; the engine detects the draft and un-drafts it itself. If the un-draft mutation fails, the merge is refused (fail loud), not half-applied.
