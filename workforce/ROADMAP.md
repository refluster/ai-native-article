...
- [ ] **Codify dev process + reviewer skills** — Author `workforce/docs/runbooks/dev-process.md` (canonical seven-phase loop). Update reviewer routine specs (`dario-review.md`, `ren-review.md` new, `aoi-review.md`) to the simplified flow (no labels). Add `maya-route-pr.md` routing routine. Bind Ren + Maya to new CCR routines. Mark `pdm-decompose.md` + `ccr-bootstrap.md` label-state-machine sections as superseded. Add `workforce/docs/follow-ups.md` index. *(Zone B docs + bindings)*
