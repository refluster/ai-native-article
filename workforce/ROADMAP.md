...
3. **Front-end agent profile migrated to the `EXEC` row family.** ✅ **Met** (landed ahead of Story 6; reconciled 2026-06-05 by Epic-012 Story 3). The `/workforce/agents/:slug` page (`apps/workforce/src/pages/AgentProfile.tsx`) reads its execution history exclusively from `PROJECT#{id}/EXEC#{ulid}` via `fetchAgentExecutions` (`GET /agents/{slug}/executions` → GSI1 `AGENT#{slug}`), not from the legacy `RUN`/`DELIV` rows. The legacy `fetchAgentDeliverables` SPA read path was removed; the `GET /agents/{slug}/deliverables` route remains as a documented legacy/historical read with no SPA caller.
