...
1. **Legacy `wf/{type}` keys removed.** Bare Secrets Manager keys (`wf/anthropic`, `wf/github`, `wf/notion`, …) are deleted after the `WfLegacyCredentialReads` CloudWatch metric has stayed at zero for ≥ 1 week. Tracked as a Story 2 follow-up PR; gated on Story 6 readiness so the credential vault UI is the only write surface.
...
4. **`article-health`-equivalent audit is clean.** A workforce-side audit (0 truncated executions, 0 orphaned `EXEC` rows, 0 cross-project leakage detections) runs green for ≥ 1 week post-cutover.
