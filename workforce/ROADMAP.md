...
Apply the Epic-010 trust boundary to enable workforce review against external repositories. Each external repo is a `PROJECT#{id}` with its own GitHub PAT, membership list, and execution ledger. The workforce stays centralised (single Lambda runner); target repos remain untouched.

- [ ] **PR1 — Project plane** — `workforce/projects/{id}/project.json` convention + `workforce:projects` validator + `workforce:projects:seed` operator script + `workforce/docs/runbooks/external-project-onboarding.md` + first project `asp-cloud` (`PSVL/asp-cloud`). *(Zone A: schema + runbook; Zone B: validator + seed script + first project file)*
- [ ] **PR2 — Nadia PdM lens + multi-project pr-review/pr-route** — Add `pr-review` (lens=product) and `pr-route` (PdM nomination_rules) bindings to Nadia; add `pr-review` / `pr-route` skill packages under `workforce/skills/` with `requires:["github.token"]`; extend `pr-review.md` / `pr-route.md` routine specs with a "Cross-project mode" section. *(Zone A: new skill SKILL.md + routine spec edits, Rule-11 for first SKILL.md; Zone B: Nadia agent.json + handlers)*
- [ ] **PR3 — Lambda handlers + SAM IAM** — `workforce/skills/pr-{route,review}/handler.ts` (Anthropic SDK → GitHub REST → `appendExecution`), agent-runner extension to pass `project_id` + injected credentials into deterministic handlers + track LLM cost; SAM IAM grant for `secretsmanager:GetSecretValue` on `wf/projects/*`. *(Zone B; potential Zone A if a new executor enum value is needed)*
- [ ] **PR4 — First end-to-end run** — Trigger `pr-route` against `PSVL/asp-cloud` PR; verify routing comment + reviewer-persona reviews + verdict comment all land on the target PR; `article-health` equivalent for execution ledger passes. *(operator action, no PR)*
