...
- [ ] **PR1 — Project plane** — `workforce/projects/{id}/project.json` file convention + `workforce:projects` validator + `workforce:projects:seed` operator script + `workforce/docs/runbooks/external-project-onboarding.md` + first project `asp-cloud` (`PSVL/asp-cloud`). The file-based seed surface for Story 1's `PROJECT#{id}` row family; Story 1 added the entity, this PR adds the authoritative source. *(Zone A: schema + runbook; Zone B: validator + seed script + first project file)*
...
- [ ] **PR3 — Lambda handlers + R-N4 amendment + SAM IAM** — `workforce/skills/pr-{route,review}/handler.ts` (Anthropic SDK → GitHub REST → `appendExecution`), agent-runner extension to pass `project_id` + injected credentials into deterministic handlers + track LLM cost; R-N4 amendment to allow `executor=lambda` with `scheduler=external|manual` (currently `lambda` requires `eventbridge` — the validator + runner gate this); SAM IAM grant for `secretsmanager:GetSecretValue` on `wf/projects/*` (already partially in place via Story 2-B). *(Zone A: R-N4 amendment in governance.md; Zone B: handlers + runner extension + IAM)*
