#!/usr/bin/env node
// One-shot registration of the VP, Operations & Reliability hire (2026-07)
...
// directly, so the create is validated and lands its AUDIT item).
//
// The petra.json + petra-system.md files in this directory are one-shot
// REGISTRATION INPUTS, not a mirror: after a successful run, the DDB row
// is authoritative and edits go through PATCH /agents/petra, never by
...
//   node workforce/seed/vp-operations/register.mjs --dry-run   # no creds needed
//
// Env:
...
// Single VP hire — Petra reports to Maya, so no ordering constraint against
// a same-round parent (she has no direct reports at registration).
const SLUGS = ["petra"];
// W-3 combined cap is USD 500/mo (governance.md §2, raised 2026-07-14 with
// standing expansion headroom). This +USD 8/mo VP sits far inside it; no cap
// raise in this round. The API re-checks the live roster aggregate at write
// time, so the true ceiling test is server-side — this constant is the
// documented ceiling, not the pre-computed sum.
const W3_CAP_USD = 500;
...
console.log(`Registering ${SLUGS.length} agent, combined budget USD ${newBudget}/mo (W-3 cap: ${W3_CAP_USD} across the whole roster — the API enforces the live aggregate per write; this round adds +USD 8/mo with no cap raise).`);
...
console.log("Done. Next: run wire-cadence.mjs to bind ops-accountability-watch, then see runbooks/agent-registration.md §after-registration.");
