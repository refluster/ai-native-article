#!/usr/bin/env node
// One-shot registration of the org-benchmark round (2026-07): nine
// positions closing the gaps unicorn-org benchmarking (Midjourney /
// Cursor / Perplexity / ElevenLabs) exposed — see
// docs/hires/org-benchmark-nine-hire-round.md.
//
// Registration goes via POST /agents (ADR-0007: DDB is the single
// authoritative store and agents-api the single writer — this script
// never touches DynamoDB directly, so every create is validated and
// lands its AUDIT item).
//
...
//   aws-vault exec <profile> -- node workforce/seed/org-benchmark-group/register.mjs
//   node workforce/seed/org-benchmark-group/register.mjs --dry-run   # no creds needed
//
// Env:
...
// reports_to. beatriz/camille/rafael report to maya, ingrid to elena,
// tomas and zoe to mateo, owen to dario, imogen to celeste — all existing
// parents — but dmitri reports to ingrid, so ingrid must precede dmitri.
const SLUGS = [
  "beatriz",
  "camille",
  "rafael",
