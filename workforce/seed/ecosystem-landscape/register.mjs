#!/usr/bin/env node
// One-shot registration of the Ecosystem & Landscape Analyst hire (2026-07)
...
//   node workforce/seed/ecosystem-landscape/register.mjs --dry-run   # no creds needed
//
// Env:
...
// Single IC hire — Bruno reports to the existing VP `mateo`, so no ordering
// constraint against a same-round parent.
const SLUGS = ["bruno"];
// W-3 combined cap is USD 500/mo (governance.md §2, raised 2026-07-14 with
// standing expansion headroom). This +USD 6/mo IC sits far inside it; no cap
...
console.log(`Registering ${SLUGS.length} agent, combined budget USD ${newBudget}/mo (W-3 cap: ${W3_CAP_USD} across the whole roster — the API enforces the live aggregate per write; this round adds +USD 6/mo with no cap raise).`);
