#!/usr/bin/env node
// One-shot registration of the Finance & Capital group (2026-06)
...
//   node workforce/seed/finance-group/register.mjs --dry-run   # no creds needed
//
// Env:
...
// reports_to, and Delphine + Corinne both report to silas.
const SLUGS = ["silas", "delphine", "corinne"];
// Pending the Zone A W-3 raise to 190 in this PR (operator-gated). The API
// re-checks the aggregate at write time, so if the cap edit hasn't merged
// the registration fails loudly instead of silently breaching 160.
